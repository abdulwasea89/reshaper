...
## Server configurationDirect link to Server configuration

You can configure server `port`and `timeout`in the Mastra instance. 

src/mastra/index.ts 
```
import { Mastra } from "@mastra/core/mastra";export const mastra = new Mastra({  // ...  server: {    port: 3000, // Defaults to 4111    timeout: 10000, // Defaults to 3 * 60 * 1000 (3 minutes)  },});
```
